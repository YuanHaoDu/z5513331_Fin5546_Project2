...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # <COMPLETE THIS PART>
    daily = ret[ret_freq_use[0]]
    monthly_cnt = daily.resample('1ME').count()
    monthly_vol = daily.resample('1ME').std()
    monthly_vol = monthly_vol[monthly_cnt > 18]
    monthly_vol_1 = monthly_vol.copy()
    monthly_vol_1 = monthly_vol_1.to_period().dropna(how='all')
    monthly_vol_1.index.name = 'Year_Month'
    monthly_vol_1.columns = [f"{i}_{cha_name}" for i in monthly_vol_1.columns]
    return monthly_vol_1
...
def merge_tables(ret, df_cha, cha_name):
    ...
    # <COMPLETE THIS PART>
    ret_1 = ret.copy()
    merge_df = ret_1['Monthly'].merge(df_cha.shift(1), left_index=True, right_index=True)
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    return merge_tables(ret, df_cha, cha_name)
