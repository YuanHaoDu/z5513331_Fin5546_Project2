...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    df_time = df.loc[start:end,].rename(columns={prc_col: tic.lower()}).dropna()
...
def daily_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    prc_return = prc / prc.shift(1) - 1
    return prc_return.dropna()
...
def monthly_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    monthly_cnt = prc.resample('1ME').count().to_period()
    monthly_last = prc.resample('1ME').last().to_period()
    monthly_returns = monthly_last / monthly_last.shift(1) - 1
    monthly_returns_cal = monthly_returns[monthly_cnt > 18].dropna()
    monthly_returns.index.name = 'Year_Month'
    return monthly_returns_cal
...
def aj_ret_dict(tickers, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    daily_return_df = pd.concat([daily_return_cal(read_prc_csv(i, start, end)) for i in tickers], axis=1, sort=True)
    monthly_return_df = pd.concat([monthly_return_cal(read_prc_csv(i, start, end)) for i in tickers], axis=1, sort=True)
    aj_ret_dict = {"Daily": daily_return_df,
                   "Monthly": monthly_return_df}
    return aj_ret_dict
